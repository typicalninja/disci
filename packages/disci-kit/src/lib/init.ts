...
import { DisckConfig, configSchema } from "./utils/constants";
...
export async function createConfigFile(at: string): Promise<DisckConfig | null> {
    try {
...
export async function createPackageJSON(dir: string) {
    ...
    try {
        await fs.writeFile(path.resolve(dir, 'package.json'), JSON.stringify(pkgJSON, null, 4))
        return true
    }
    catch {
        return null
...
export async function installRequiredPackages(dir: string, config: DisckConfig, packageManager?: string) {
    const pm = packageManager ? ({ name: packageManager }) : ((await whichpm(process.cwd())) || "npm");
    ...
    try {
        const { stdout } = await exec(`${pm.name} install ${args.join(" ")}`, { cwd: dir });
        console.log(stdout)
        return true;
    }
    catch(e) {
...
export async function init(directory: string, force = false, noInstall = false, packageManager?: string) {
    ...
    if(await directoryExists(destinationDirectory)) {
         if(!(await directoryEmpty(destinationDirectory)) && !force) {
            return logger.error(`Destination directory is not empty, delete it or use --force`)
        }
        else {
    ...
    const config = await createConfigFile(path.join(destinationDirectory, 'kit.json'));
    if(!config) return;

    const eDirectoryCS = await createEssentialDirectories(path.resolve(destinationDirectory, config.srcDir))
    if(!eDirectoryCS) return;

    if(noInstall) {
        logger.warn("Package installation disabled... please install the required packages manually.")
    }
    else {
