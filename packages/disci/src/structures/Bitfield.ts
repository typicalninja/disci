	...
	static resolve(bit: BitFieldResolvable): bigint {
		switch (typeof bit) {
			...
			default:
				...
				} else throw new DisciTypeError(TypeErrorsMessages.ExpectedParameter("bit", "bitFieldResolvable", typeof bit))
		}
	}
}
	...
	*[Symbol.iterator]() {
	...
	get array() {
		return Object.keys(PermissionFlagsBits).filter((flag: unknown) =>
			this.has(PermissionsBitField.Flags[flag as keyof typeof PermissionFlagsBits]),
		)
	}
}
