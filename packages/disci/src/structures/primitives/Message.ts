...
export default class Message implements IBase {
  ...
  async pin(): Promise<void> {
    await this.handler.api.put<void>(Routes.channelPin(this.channelId, this.id))
  }
  /**
   * Unpin this message
   */
  ...
  static resolveMessageParams(params: CreateMessageParams) {
    ...
    if (params.embeds) {
      if (!Array.isArray(params.embeds))
        throw new DisciTypeError(TypeErrorsMessages.ExpectedParameter(`message.embeds`, 'array', typeof params.embeds))
