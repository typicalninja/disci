...
export abstract class BaseInteraction implements IBase {
	 ...
	 */
	constructor(handler: InteractionHandler, readonly RawInteractionData: APIInteraction) {
	...
	deferResponse({ fetchReply = false, ephemeral = false } = {}): Promise<this> | Promise<Message> {
		if (this.responded) throw new DisciError(`This Interaction already timed out or has been replied to`)
