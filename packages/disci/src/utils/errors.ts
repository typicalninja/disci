...
export const TypeErrorsMessages = {
	ExpectedParameter: (value: string, expected: string, received?: string) =>
		`Expected ${value} as ${expected} ${
			received ? `but received ${received}` : ""
		}`,
	ParameterRequired: (parameter: string) =>
		`Parameter ${parameter} is required and must not be null/undefined`,
	PropertyNotFound: (prop: string) => `No ${prop} Found`,
	UnSupportedFeature: (feature: string) =>
		`${feature} is unsupported currently`,
};
