...
> {
		...
		return new Promise((resolve, reject) => {
			switch (rawInteraction.type) {
				...
				default: {
					...
					const responseEvent =
						InternalEventNames.interactionResponse + interaction.id;
					...
					const responseHandler = (response: APIInteractionResponse) => {
						...
						this.emit(
							"debug",
							`[InteractionHandler::respondlistner] Responding to request [${interaction.id}] via http`,
							response,
						);
						resolve(response);
					};
	...
	async handleRequest(request: GenericRequest) {
		...
		if (
			...
			throw new Error("Required request properties not found");
